...
OFF_TOPIC_REPLY = (
    ...
    'infrastructure projects: cost and schedule overruns, risk scores, '
    'warnings, sectors/ministries/states, the predictive models and the '
    'underlying public data.\n\nTry: "top 10 riskiest projects", "cost '
    'overrun in Railways", "tell me about project N24001451", or "help".')

# Vocabulary that marks a question as PAIMANA-related. Permissive by design:
# the Gemini system prompt and the rules engine handle everything else.
_PAIMANA_WORDS = {
    'paimana', 'mospi', 'ocms', 'project', 'projects', 'infrastructure',
    'cost', 'costs', 'overrun', 'overruns', 'escalation', 'budget',
    'expenditure', 'spending', 'spend', 'crore', 'crores', 'lakh', 'delay',
    'delays', 'delayed', 'late', 'schedule', 'scheduled', 'slip', 'timeline',
    'milestone', 'milestones', 'completion', 'sector', 'sectors', 'ministry',
    'ministries', 'state', 'states', 'agency', 'agencies', 'railway',
    'railways', 'road', 'roads', 'highway', 'highways', 'transport', 'power',
    'energy', 'petroleum', 'coal', 'steel', 'mines', 'mining', 'port',
    'ports', 'aviation', 'airport', 'telecom', 'telecommunication', 'urban',
    'water', 'sanitation', 'health', 'education', 'defence', 'defense',
    'renewable', 'atomic', 'shipping', 'risk', 'risks', 'risky', 'warning',
    'warnings', 'anomaly', 'anomalies', 'forecast', 'forecasts', 'trend',
    'trends', 'model', 'models', 'accuracy', 'auc', 'shap', 'driver',
    'drivers', 'benchmark', 'benchmarking', 'peer', 'peers', 'progress',
    'physical', 'dashboard', 'data', 'quality', 'sources', 'source',
    'recommendation', 'recommendations', 'what-if', 'whatif', 'assistant',
    'sih', 'hackathon', 'panel', 'database',
}
_PHRASES = (
    'what can you', 'who are you', 'how do you work', 'are you', 'your name',
    'good morning', 'good evening', 'good afternoon', 'thank you',
)


def _is_paimana_related(question: str) -> bool:
    """True if the question is about PAIMANA (or is a greeting/help request).
    Everything else is refused before reaching any answer engine."""
    ql = question.lower()
    if any(p in ql for p in _PHRASES):
        return True
    if ql.split()[0:1] and ql.strip(' !?.').split()[0] in (
            'hi', 'hello', 'hey', 'namaste', 'yo', 'greetings', 'thanks',
            'thankyou', 'help', 'hii', 'hiii'):
        return True
    tokens = set(''.join(c if c.isalnum() else ' ' for c in ql).split())
    return bool(tokens & _PAIMANA_WORDS)
...
def answer(q: str, card=None) -> dict:
    """Dispatch the question. Off-topic questions are refused first; then
    Gemini (database-grounded) answers if configured, otherwise the
    deterministic rule-based engine. Any Gemini failure falls back to the
    rules engine, so answers never break."""
    from . import gemini
    question = (q or '').strip()
    if question and not _is_paimana_related(question):
        return {'answer': OFF_TOPIC_REPLY, 'intent': 'off_topic',
                'engine': 'rules'}
    if gemini.enabled():
        g = gemini.answer_with_gemini(question)
        if g:
            return g
    res = _rules(question, card)
    res['engine'] = 'rules'
    return res


def _rules(q: str, card=None) -> dict:
    q = (q or '').strip()
    ql = q.lower()
    if not ql:
